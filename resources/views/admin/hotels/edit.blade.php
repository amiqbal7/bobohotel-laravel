            ...
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
                @if ($errors->any())
                    @foreach ($errors->all() as $error)
                        <div class="py-3 grid gap-3 w-full rounded-3xl bg-red-500 text-white">
                            {{ $error }}
                        </div>
                    @endforeach
                @endif
                ...
                <form method="POST" action="{{ route('admin.hotels.update', $hotel) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input value="{{ $hotel->name }}" id="name" class="block mt-1 w-full"
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input value="{{ $hotel->link_gmaps }}" id="link_gmaps" class="block mt-1 w-full"
                            type="text" name="link_gmaps" autofocus autocomplete="link_gmaps" />
                        <x-input-error :messages="$errors->get('link_gmaps')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="address" :value="__('address')" />
                        <textarea name="address" id="address" cols="30" rows="5" class="border border-slate-300 rounded-xl w-full">{{ old('address', $hotel->address) }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input value="{{ $hotel->star_level }}" id="star_level" class="block mt-1 w-full"
